...
const QuoteForm = ({ projectConfig }) => {
    ...
    return (
        <div className="relative rounded-lg bg-white p-8 shadow-lg sm:p-12">
            <AnimatedTitle title={`${useTranslate('qetAQuote')}`} className='text-primary text-2xl md:text-4xl mb-10 text-center' />
            <form onSubmit={handleSubmit(SubmitContact)}>
                ...
                <div className="mb-6">
                    <input
                        ...
                        {...register('email', {
                            ...
                            pattern: { value: /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/, message: 'The email is invalid' }
                        })}
                    />
                    {errors.email &&
                ...
                <div className="mb-6">
                    ...
                    {errors.companySubject &&
                        <span className="text-red-500">{errors.companySubject.message}</span>
                    }
                </div>
                <div className="mb-6">
                    <textarea
