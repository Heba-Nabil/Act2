...
import ProductsTitle from "@/components/productsPage/ProductsTitle";
...
export async function generateMetadata({ params: { locale } }) {
  ...
  const [websiteDataRes, pageSeoRes] = await Promise.all([
    ...
    fetch(
      `${process.env.BASE_URL}/${process.env.PROJECT_CODE}/SEO/${locale}/ProductsPage/Index`, { cache: "no-store" }
    )
  ]);
...
async function ProductsPage({ params: { locale } }) {
  const { products, marqueeProduct } = await fetchProducts(locale)
  const crumbs = [
    { title: "Home", href: "/" },
    { title: "Products", href: "" },
  ];
  ...
  return (
    <>
      ...
      <Breadcrumbs className="w-full " crumbs={crumbs} />
        {/* <Marquee data={marqueeProduct} className="opacity-10" /> */}
